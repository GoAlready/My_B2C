...
<body>
 <div class="margin clearfix">
   <div class="border clearfix">
       <span class="l_f">
        <a href="{{route('addPrivilege')}}" id="Competence_add" class="btn btn-warning" title="添加权限"><i class="fa fa-plus"></i> 添加权限</a>
        <a  onclick="dels()" class="btn btn-danger"><i class="fa fa-trash"></i> 批量删除</a>
       </span>
			...
			<tr>
			 ...
			 @foreach($user as $v)
			  <tr>
				...
				<td>{{$v['pri_name']}}</td>
				<td>{{$v['par']['pri_name']}}</td>
				<td>{{$v['path']}}</td>
				<td>
                 <a title="编辑" href="editPrivilege?id={{$v['id']}}"  class="btn btn-xs btn-info" ><i class="fa fa-edit bigger-120"></i></a>        
...
function Competence_del(obj,id){
	layer.confirm('删除会删除相应子分类 确定要删除吗？',function(index){
		$(obj).parents("tr").remove();
		layer.msg('已删除!',{icon:1,time:1000});
		$.ajax("/root/delPrivilege?id="+id);
	});
}
/*修改权限*/
    ...
    var checked = [];
       $('input:checkbox:checked').each(function() {
        ...
        if(checked.length>=1){
            ...
            $.ajax("/root/delPrivilege?id="+checked[i]);
            }
