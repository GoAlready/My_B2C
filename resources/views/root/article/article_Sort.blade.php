...
<div class="margin clearfix">
 <div class="sort_style">
  <div class="border clearfix">
       <span class="l_f">
        <a href="{{route('addType')}}"id="add_page" class="btn btn-warning"><i class="fa fa-plus"></i> 添加分类</a>
       ...
       <thead>
        ...
        @foreach($type as $v)
         <tr>
          ...
          <td>{{$v['id']}}</td>
          <td>{{$v['type_name']}}</td>
          <td class="displayPart" displayLength="60">{{$v['type_introduce']}}</td>
          ...
          @endif
          <td class="td-manage">   
           <a title="编辑" href="/root/editType?id={{$v['id']}}"  class="btn btn-xs btn-info" ><i class="fa fa-edit bigger-120"></i></a>      
    ...
    var checked = [];
       $('input:checkbox:checked').each(function() {
        ...
        for(let i=0;i<checked.length;i++){
            $.ajax("/root/delType?id="+checked[i]);
        }
